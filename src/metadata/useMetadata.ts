...
export function useCreateMetadataContext() {
	...
	const setZoomPreview = useCallback(
		(zoomPreview: boolean) => {
			setState((d) => {
	...
	const setImageTab = useCallback(
		(image: ImageItem, tab: "image" | "config" | "gen") => {
...
export async function loadImage(
	item?: string | DataTransfer | Blob | Uint8Array<ArrayBufferLike>,
...
export async function getMetaDataFromBuffer(
